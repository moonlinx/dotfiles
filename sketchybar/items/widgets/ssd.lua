...
system_indicator:subscribe("swap_system_info", function(env)
	local currently_on = system_indicator:query().icon.value == icons.zen.on
	system_indicator:set({
		icon = currently_on and icons.zen.off or icons.zen.on,
	})
end)
...
system_info_swap:subscribe("swap_system_info", function(env)
	...
	if drawing then
		...
		sbar.set("widgets.time_machine", { drawing = false })
		sbar.set("widgets.onepassword", { drawing = false })
		...
		sbar.set("widgets.time_machine", { drawing = true })
		sbar.set("widgets.onepassword", { drawing = true })
