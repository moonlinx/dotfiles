...
filters = string.format(
	"fps=%s,scale='trunc(ih*dar/2)*2:trunc(ih/2)*2',setsar=1/1,scale=%s:-1:flags=spline",
	fps,
	options.rez
) --change spline to lanczos depending on preference
...
function make_gif_internal(burn_subtitles)
	...
	args = string.format(
		'ffmpeg -v warning -ss %s -t %s -i "%s" -vf "%s,palettegen" -y "%s"',
		position,
		duration,
		esc(pathname),
		esc(trim_filters),
	...
	args = string.format(
		'ffmpeg -v warning -ss %s %s -t %s -i "%s" -i "%s" -lavfi "%s [x]; [x][1:v] paletteuse" -y "%s"',
		position,
		copyts,
		duration,
		esc(pathname),
