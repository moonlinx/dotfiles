local options = {
	shader_path = "~/.config/mpv/shaders",
	shader_profiles = "",
...
local function create_property_selection(
	name,
	property,
	options,
	off_or_default_option,
	include_default_item,
	include_custom_item
)
	...
	local function create_option_item(name, value)
		...
		return {
			title = name,
			active = is_active,
			value = is_active and off_or_default_option and command("set-property", property, off_or_default_option)
				or command("set-property", property, value),
		}
	end
...
local function create_property_number_adjustment(name, property, increment, min, max)
	...
	local function create_hint()
		...
		return tonumber(current_property[property])
				and string.format("%.3f", tonumber(current_property[property])):gsub("%.?0*$", "")
			or current_property[property]
	end
...
local function create_aspect_menu()
	...
	local function create_aspect_profile_item(name, profile_aspect_value)
		...
		return {
			title = name,
			active = is_active,
			value = is_active and command("set-property", "video-aspect-override", "-1")
				or command("set-property", "video-aspect-override", tostring(profile_aspect_value)),
		}
	end
	...
	if not default_profile_override and options.include_default_aspect_profile then
		table.insert(
			aspect_profiles,
			1,
			create_aspect_profile_item(
				options.default_aspect_profile_name,
...
local function create_deband_menu()
	...
	local function create_deband_profile_item(name, profile_iterations, profile_threshold, profile_range, profile_grain)
		local is_active = (
			tonumber(profile_iterations) == tonumber(current_property["deband-iterations"])
			and tonumber(profile_threshold) == tonumber(current_property["deband-threshold"])
			and tonumber(profile_range) == tonumber(current_property["deband-range"])
			and tonumber(profile_grain) == tonumber(current_property["deband-grain"])
		)
		...
		return {
			...
			value = is_active and command("toggle-property", "deband")
				or command("apply-deband-profile", profile_iterations, profile_threshold, profile_range, profile_grain),
		}
	end
	...
	for deband_profile in options.deband_profiles:gmatch("([^;]+)") do
		...
		if name and settings then
			local profile_iterations, profile_threshold, profile_range, profile_grain =
				settings:match("([^,]+),([^,]+),([^,]+),([^,]+)")

			if profile_iterations and profile_threshold and profile_range and profile_grain then
				local is_default = tonumber(profile_iterations) == tonumber(default_property["deband-iterations"])
					and tonumber(profile_threshold) == tonumber(default_property["deband-threshold"])
					and tonumber(profile_range) == tonumber(default_property["deband-range"])
					and tonumber(profile_grain) == tonumber(default_property["deband-grain"])
				...
				table.insert(
					deband_profile_items,
					create_deband_profile_item(
						name,
						profile_iterations,
						profile_threshold,
						profile_range,
	...
	if not default_profile_override and options.include_default_deband_profile then
		table.insert(
			...
			1,
			create_deband_profile_item(
				options.default_deband_profile_name,
				default_property["deband-iterations"],
				default_property["deband-threshold"],
				default_property["deband-range"],
	...
	for _, item in ipairs({
		create_property_toggle("Enabled", "deband"),
		...
		create_property_number_adjustment("Grain", "deband-grain", 1, 0, 4096),
	}) do
...
local function create_color_menu()
	...
	local profile_hint
	local profile_match = false
	local function create_color_profile_item(
		name,
		profile_brightness,
		profile_contrast,
		profile_saturation,
		...
		local is_active = (
			tonumber(profile_brightness) == tonumber(current_property["brightness"])
			and tonumber(profile_contrast) == tonumber(current_property["contrast"])
			and tonumber(profile_saturation) == tonumber(current_property["saturation"])
			and tonumber(profile_gamma) == tonumber(current_property["gamma"])
			and tonumber(profile_hue) == tonumber(current_property["hue"])
		)
		...
		return {
			title = name,
			active = is_active,
			value = is_active and command("clear-color") or command(
				"apply-color-profile",
				profile_brightness,
				profile_contrast,
				profile_saturation,
	...
	for color_profile in options.color_profiles:gmatch("([^;]+)") do
		...
		if name and settings then
			local profile_brightness, profile_contrast, profile_saturation, profile_gamma, profile_hue =
				settings:match("([^,]+),([^,]+),([^,]+),([^,]+),([^,]+)")

			if profile_brightness and profile_contrast and profile_saturation and profile_gamma and profile_hue then
				local is_default = tonumber(profile_brightness) == tonumber(default_property["brightness"])
					and tonumber(profile_contrast) == tonumber(default_property["contrast"])
					and tonumber(profile_saturation) == tonumber(default_property["saturation"])
					and tonumber(profile_gamma) == tonumber(default_property["gamma"])
					and tonumber(profile_hue) == tonumber(default_property["hue"])
	...
	if not default_profile_override and options.include_default_color_profile then
		table.insert(
			...
			1,
			create_color_profile_item(
				options.default_color_profile_name,
	...
	if options.include_custom_color_profile then
		local is_original = tonumber(current_property["brightness"]) == 0
			and tonumber(current_property["contrast"]) == 0
			and tonumber(current_property["saturation"]) == 0
			and tonumber(current_property["gamma"]) == 0
			and tonumber(current_property["hue"]) == 0
		if not is_original and not profile_match then
			profile_hint = "Custom"
		end
...
local function create_scale_number_adjustments(property)
	return create_property_number_adjustment("Antiring", property .. "-antiring", 0.005, 0, 1),
		create_property_number_adjustment("Blur", property .. "-blur", 0.005),
		create_property_number_adjustment("Clamp", property .. "-clamp", 0.005, 0, 1),
		create_property_number_adjustment("Radius", property .. "-radius", 0.005, 0.5, 16),
		create_property_number_adjustment("Taper", property .. "-taper", 0.005, 0, 1)
end
...
local function create_scale_menu()
	...
	table.insert(scale_items, {
		title = "Upscale",
		items = {
			create_property_selection("Filters", "scale", get_scale_filters()),
	...
	table.insert(scale_items, {
		title = "Downscale",
		items = {
			create_property_selection("Filters", "dscale", get_scale_filters(), ""),
	...
	table.insert(scale_items, {
		title = "Chromascale",
		items = {
			create_property_selection("Filters", "cscale", get_scale_filters(), ""),
	...
	table.insert(scale_items, {
		title = "Temporalscale",
		items = {
			create_property_selection("Filters", "tscale", get_tscale_filters()),
	...
	for _, item in ipairs({
		create_property_toggle("Linear upscaling", "linear-upscaling"),
...
local function move_shader(shader, direction_or_index)
	...
	local new_position = (direction_or_index == "up" and target_index - 1)
		or (direction_or_index == "down" and target_index + 1)
		or tonumber(direction_or_index)
...
local function create_shader_menu()
	...
	local function create_shader_profile_item(name, profile_shader_list)
		...
		return {
			title = name,
			active = is_active,
			value = is_active and command("clear-shaders")
				or command("set-property", "glsl-shaders", table.unpack(profile_shader_list)),
		}
	end
	...
	for shader_profile in options.shader_profiles:gmatch("([^;]+)") do
		...
		if name and shaders then
			...
			local is_default = compare_shaders(
				get_active_shaders(profile_shader_list),
...
local function create_menu_data()
	...
	local advanced_items = {
		title = "Advanced",
		items = {
			create_property_selection("Deinterlace", "deinterlace", deinterlace_options),
			create_dither_menu(),
			create_property_selection("Hardware decoding", "hwdec", hwdec_options),
			create_scale_menu(),
